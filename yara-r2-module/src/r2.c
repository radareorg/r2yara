...
define_function(import_isss) // ordinal, bind, type, name
{
	...
	json_array_foreach (list, index, value) {
		...
		if (arg_ordinal == ordinal) {
			result += 1;
		}
		if (arg_bind && bind && strcasecmp (arg_bind, bind) == 0) {
			result += 1;
		}
		if (arg_type && type && strcasecmp (arg_type, type) == 0) {
			result += 1;
		}
		if (arg_name && name && strcasecmp (arg_name, name) == 0) {
			result += 1;
		}
		if (result == 4) {
			break;
...
define_function(import_issr) // ordinal, bind, type, name
{
	...
	json_array_foreach (list, index, value) {
		...
		if (name && yr_re_match (arg_name, name) > 0) {
			result += 1;
		}
		if (result == 4) {
			break;
...
define_function(import_isrs) // ordinal, bind, type, name
{
	...
	json_array_foreach (list, index, value) {
		...
		if (arg_bind && strcasecmp (arg_bind, bind) == 0) {
			result += 1;
		}
		if (arg_name && strcasecmp (arg_name, name) == 0) {
			result += 1;
		}
		if (yr_re_match (arg_type, type) > 0) {
			result += 1;
		}
		if (result == 4) {
			break;
...
define_function(import_isrr) // ordinal, bind, type, name
{
	...
	json_array_foreach (list, index, value) {
		...
		if (yr_re_match (arg_name, name) > 0) {
			result += 1;
		}
		if (result == 4) {
			break;
...
define_function(import_irss) // ordinal, bind, type, name
{
	...
	json_array_foreach (list, index, value) {
		...
		if (arg_type && strcasecmp (arg_type, type) == 0) {
			result += 1;
		}
		if (arg_name && strcasecmp (arg_name, name) == 0) {
			result += 1;
		}
		if (yr_re_match (arg_bind, bind) > 0) {
			result += 1;
		}
		if (result == 4) {
			break;
...
define_function(import_irrr) // ordinal, bind, type, name
{
	...
	json_array_foreach (list, index, value) {
		...
		if (arg_ordinal == -1 || arg_ordinal == ordinal) {
			result += 1;
		}
		if (yr_re_match (arg_bind, bind) > 0) {
			result += 1;
...

{
	...
	json_array_foreach (list, index, value) {
		...
		if (arg_flags && strcasecmp (arg_flags, flags) == 0) {
			result += 1;
		}
...
define_function(section_sr) // name,flags
{
	...
	json_array_foreach (list, index, value) {
		...
		if (yr_re_match (arg_flags, flags) > 0) {
			result += 1;
		}
		if (result == 2) {
			break;
...

{
	...
	json_array_foreach (list, index, value) {
		...
		if (arg_lang && strcasecmp (arg_lang, lang) == 0) {
			result += 1;
		}
...
define_function(resource_sr) // type, lang
{
	...
	json_array_foreach (list, index, value) {
		...
		if (yr_re_match (arg_lang, lang) > 0) {
			result += 1;
		}
		if (result == 2) {
			break;
...
define_function(hash_ss) // name, hash
{
	...
	json_array_foreach (list, index, value) {
		...
		if (arg_hash && strcasecmp (arg_hash, hash) == 0) {
			result += 1;
		}
...
int module_load(
	...
	R2Pipe *r2 = NULL;
	char hash_names[NUMBER_OF_HASHES][16] = { "md5", "sha1", "sha256", "sha384", "sha512", "crc16", "crc24", "crc32", "crc32c", "crc32ecma267", "md4", "xor", "xorpair", "parity", "entropy", "hamdist", "pcprint", "mod255", "xxhash", "adler32", "luhn", "crc8smbus", "crc15can", "crc16hdlc", "crc16usb", "crc16citt" };
